...
export default function ChatHistory() {
  ...
  const { connectWebSocket, onChatUpdated } = useWebSocket();
  ...
  useEffect(() => {
    ...
    const unsubscribe = onChatUpdated((updatedChat: IChat) => {
      setChats((prevChats) =>
        prevChats.map((chat) =>
  ...
  }, [connectWebSocket, onChatUpdated]);
  ...
  return (
    <aside className={styles.history}>
      <div className={styles["history-settings"]}>
        <button
          className={styles["history-settings-add-chat"]}
          onClick={createChat}
        >
          <AddChatIcon />
        </button>
